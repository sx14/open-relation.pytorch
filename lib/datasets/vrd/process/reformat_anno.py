...
def reformat_anno(dataset_config):
    org_anno_root = dataset_config['dirty_anno_root']
    dst_anno_root = dataset_config['clean_anno_root']

    # load vrd label list
    obj_label_list_path = os.path.join(dataset_config['obj_raw_label_path'])
    obj_ind2label = load_list(obj_label_list_path)

    pre_label_list_path = os.path.join(dataset_config['pre_raw_label_path'])
