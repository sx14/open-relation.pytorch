...
class vg_lsj_rela(imdb):
    ...
    def _load_image_set_index(self):
        ...
        with open(image_set_file) as f:
            ...
            for index in raw_img_indexes:
                index = index.strip()
                roi = self._load_vg_annotation(index)
                if len(roi['boxes']) > 0:
    ...
    def gt_roidb(self):
        ...
        """
        cache_file = os.path.join(self.cache_path, self.name + '_gt_rlt_roidb.pkl')
    ...
    def _load_vg_annotation(self, index):
        anno_path = os.path.join(self._data_path, 'anno', index+'.json')
        with open(anno_path, 'r') as f:
            raw_anno = json.load(f)
        raw_rlts = raw_anno['relationships']

        sbj_boxes = np.zeros((len(raw_rlts), 4), dtype=np.uint16)
        obj_boxes = np.zeros((len(raw_rlts), 4), dtype=np.uint16)
        pre_boxes = np.zeros((len(raw_rlts), 4), dtype=np.uint16)

        sbj_classes = np.zeros((len(raw_rlts)), dtype=np.int32)
        obj_classes = np.zeros((len(raw_rlts)), dtype=np.int32)
        pre_classes = np.zeros((len(raw_rlts)), dtype=np.int32)

        ishards = np.zeros((len(raw_rlts)), dtype=np.int32)
        seg_areas = np.zeros((len(raw_rlts)), dtype=np.float32)
        overlaps = np.zeros((len(raw_rlts), self.num_classes), dtype=np.float32)
        ...
        for i, raw_rlt in enumerate(raw_rlts):
            ...
            overlaps[i, pre_cls] = 1.0
            seg_areas[i] = (pre_box[2] - pre_box[0] + 1) * (pre_box[3] - pre_box[1] + 1)

        overlaps = scipy.sparse.csr_matrix(overlaps)

        return {'sbj_boxes': sbj_boxes,
                'sbj_classes': sbj_classes,
                'obj_boxes': obj_boxes,
                'obj_classes': obj_classes,

                'boxes': pre_boxes,
                'gt_classes': pre_classes,
