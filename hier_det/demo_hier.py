...
if __name__ == '__main__':
    ...
    if args.net == 'vgg16':
        ...
        labelconf = HierLabelConfig(args.dataset, 'object')
        label_vec_path = labelconf.label_vec_path()
        hierRCNN = resnet(objnet, label_vec_path, 101, pretrained=False, class_agnostic=args.class_agnostic)
    else:
    ...
    while (num_images > 0):
            ...
            if cfg.TRAIN.BBOX_NORMALIZE_TARGETS_PRECOMPUTED:
                ...
                if args.class_agnostic:
                    ...
                    if args.cuda > 0:
                        ...
                        box_deltas = box_deltas.view(-1, 4) * torch.FloatTensor(cfg.TRAIN.BBOX_NORMALIZE_STDS) \
                                     + torch.FloatTensor(cfg.TRAIN.BBOX_NORMALIZE_MEANS)
                    box_deltas = box_deltas.view(1, -1, 4 * len(objnet.get_all_labels()))
        ...
        misc_tic = time.time()
        '''
        for j in xrange(1, len(objnet.get_all_labels())):
            ...
            if inds.numel() > 0:
                ...
                if vis:
                    im2show = vis_detections(im2show, objnet.get_all_labels()[j], cls_dets.cpu().numpy(), 0.5)
        '''
        misc_toc = time.time()
