    ...
    for i in range(len(anno_list)):
        ...
        for rlt in anno_rlts:
            ...
            for j, thing in enumerate(things):
                ...
                xmax = int(thing['xmax'])
                ymax = int(thing['ymax'])
                if labelnets[j].get_node_by_name(thing['name']) is None:
                    pp = 1
...
if __name__ == '__main__':
    ...
    train_anno_list_path = os.path.join(vg_config['ImageSets'], 'trainval.txt')
    ...
    test_anno_list_path = os.path.join(vg_config['ImageSets'], 'test.txt')
